...
CONFIG = {
    ...
    "STRATEGY_5": {  # Advanced crypto-futures MTF strategy
        ...
        # Sizing and risk
        "RISK_USD": float(os.getenv("S5_RISK_USD", "0.50")),
        # Stops
        "STOP_ATR_MULT": float(os.getenv("S5_STOP_ATR_MULT", "1.0")),
        "STOP_MIN_ATR_MULT": float(os.getenv("S5_STOP_MIN_ATR_MULT", "1.0")),
        "STOP_BUFFER_PCT": float(os.getenv("S5_STOP_BUFFER_PCT", "0.0025")),
        # Volatility guard (ATR/close in percent)
...
def get_renko_data(df_raw: pd.DataFrame, symbol: str) -> Optional[pd.DataFrame]:
    ...
    - Uses ATR(14) from the raw data for brick_size
    - Ensures a 'date' column exists for stocktrends
    - Tries get_ohlc_data() first (preferred in stocktrends >=0.1.6), falls back to get_bricks()
    - Returns a DataFrame with columns: open, high, low, close, volume (volume=0)
    """
    ...
    try:
        # 1) Determine brick size from ATR
        atr_period = CONFIG.get("ATR_LENGTH", 14)
        atr_series = atr(df_raw, length=atr_period)
        brick_size = safe_last(atr_series)
        if brick_size <= 0:
            log.warning(f"Calculated Renko brick size for {symbol} is {brick_size:.6f}. Cannot generate Renko chart.")
            return None
        log.info(f"Generating Renko chart for {symbol} with ATR({atr_period}) brick size: {brick_size:.6f}")

        # 2) Prepare DataFrame for stocktrends: needs a 'date' column
        df_for_renko = df_raw.copy()
        # stocktrends expects columns: 'date','open','high','low','close'
        if 'date' not in df_for_renko.columns:
            # Use the index as date; ensure tz-aware UTC
            idx = df_for_renko.index
            if getattr(idx, 'tz', None) is None:
                # index is naive; localize to UTC
                date_col = pd.to_datetime(idx).tz_localize('UTC')
            else:
                date_col = pd.to_datetime(idx).tz_convert('UTC')
            df_for_renko = df_for_renko.reset_index(drop=False)
            df_for_renko.insert(0, 'date', date_col)
        # Make sure columns are correctly named
        df_for_renko.rename(columns={
            'Open': 'open', 'High': 'high', 'Low': 'low', 'Close': 'close', 'Volume': 'volume'
        }, inplace=True)
        # limit to necessary columns; keep volume if present
        keep_cols = [c for c in ['date', 'open', 'high', 'low', 'close', 'volume'] if c in df_for_renko.columns]
        df_for_renko = df_for_renko[keep_cols]

        # 3) Build Renko
        renko = Renko(df_for_renko)
        # Prefer property assignment; fall back to legacy setter if available
        try:
            renko.brick_size = brick_size
        except Exception:
            try:
                renko.set_brick_size(box_size=brick_size, auto=False)  # legacy
            except Exception:
                pass
        ...
        try:
            ...
            try:
                renko_df = renko.get_bricks()
            except Exception:
                renko_df = None

        if renko_df is None or renko_df.empty:
            log.warning(f"Renko chart generation for {symbol} resulted in empty data.")
            return None
        ...
        if getattr(renko_df.index, 'tz', None) is None:
            ...
            renko_df.index = pd.to_datetime(renko_df.index).tz_convert('UTC')
        renko_df.index.name = 'close_time'

        # stocktrends may return different case; normalize names
        col_map = {c.lower(): c for c in renko_df.columns}
        # Create a lowercase view
        lower_cols = [c.lower() for c in renko_df.columns]
        data = {}
        for name in ['open', 'high', 'low', 'close']:
            if name in lower_cols:
                data[name] = renko_df[col_map[name]]
            else:
                raise KeyError(f"Renko output missing '{name}' column")
        data['volume'] = pd.Series(0, index=renko_df.index)

        out = pd.DataFrame(data, index=renko_df.index)
        return out
...
def calculate_all_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """
    Accepts df (OHLCV) and returns df with added indicator columns.
    - Avoids over-strict lookback gating: computes what it can and leaves NaNs where insufficient.
    - Uses active STRATEGY_MODE to decide which families to compute.
    """
    if df is None or df.empty:
        return df
    ...
    # ---- Always-on base indicators (safe with short lookbacks) ----
    try:
        out['atr'] = atr(out, CONFIG.get("ATR_LENGTH", 14))
    except Exception:
        log.exception("ATR calculation failed")
    try:
        adx(out, period=CONFIG.get('ADX_PERIOD', 14))
    except Exception:
        log.exception("ADX calculation failed")
    try:
        out['rsi'] = rsi(out['close'], length=CONFIG.get('RSI_LEN', 14))
    except Exception:
    ...
    if 0 in modes or 1 in modes:
        try:
            s1 = CONFIG['STRATEGY_1']
            out['s1_bbu'], out['s1_bbl'] = bollinger_bands(out['close'], s1.get('BB_LENGTH', 20), s1.get('BB_STD', 2.0))
        except Exception:
            log.exception("BBands calculation failed")

    # ---- Strategy 2 (ST) ----
    if 0 in modes or 2 in modes:
        try:
            s2 = CONFIG['STRATEGY_2']
            out['s2_st'], out['s2_st_dir'] = supertrend(out, period=s2.get('SUPERTREND_PERIOD', 7), multiplier=s2.get('SUPERTREND_MULTIPLIER', 3.0))
        except Exception:
            log.exception("S2 SuperTrend calculation failed")

    # ---- Strategy 3 (MAs) ----
    if 0 in modes or 3 in modes:
        try:
            s3 = CONFIG['STRATEGY_3']
            out['s3_ma_fast'] = sma(out['close'], s3.get('FAST_MA', 9))
            out['s3_ma_slow'] = sma(out['close'], s3.get('SLOW_MA', 21))
        except Exception:
            log.exception("S3 MAs calculation failed")

    # ---- Strategy 4 (3x ST) ----
    if 0 in modes or 4 in modes:
        try:
            s4 = CONFIG['STRATEGY_4']
            out['s4_st1'], out['s4_st1_dir'] = supertrend(out, period=s4.get('ST1_PERIOD', 12), multiplier=s4.get('ST1_MULT', 3.0))
            out['s4_st2'], out['s4_st2_dir'] = supertrend(out, period=s4.get('ST2_PERIOD', 11), multiplier=s4.get('ST2_MULT', 2.0))
            out['s4_st3'], out['s4_st3_dir'] = supertrend(out, period=s4.get('ST3_PERIOD', 10), multiplier=s4.get('ST3_MULT', 1.4))
            if s4.get('EMA_FILTER_ENABLED', False) and s4.get('EMA_FILTER_PERIOD', 0) > 0:
                out['s4_ema_filter'] = ema(out['close'], length=s4.get('EMA_FILTER_PERIOD', 200))
        except Exception:
            log.exception("S4 SuperTrends/EMA calculation failed")
...
async def evaluate_and_enter(symbol: str):
    ...
    async with lock:
        ...
        try:
            ...
            # --- S5 (MTF) runs independently and fetches its own TF data ---
            if run_s5:
                await evaluate_strategy_5(symbol)
...
async def evaluate_strategy_5(symbol: str):
    """
    Strategy 5: Advanced crypto-futures MTF (H1 trend, M15 execution).
    - H1 trend filter: EMA 21/55 + SuperTrend(10,3)
    - M15 execution: EMA 21/55, ATR(14), RSI(14), volume spike vs 10-bar average
    - Entry: limit at M15 EMA21 (signal candle close as reference)
    - SL: structure/ATR hybrid with buffer
    - Position sizing: fixed RISK_USD
    """
    try:
        s5 = CONFIG.get("STRATEGY_5", {})
        # --- Fetch higher timeframe and execution timeframe data ---
        df_h1 = await asyncio.to_thread(fetch_klines_sync, symbol, "1h", 400)
        df_m15 = await asyncio.to_thread(fetch_klines_sync, symbol, "15m", 400)

        if df_h1 is None or df_h1.empty or df_m15 is None or df_m15.empty:
            log.warning(f"S5: Missing data for {symbol} (H1 or M15). Skipping.")
            return

        # H1 indicators
        h1_ema_s = ema(df_h1['close'], s5.get('H1_EMA_SHORT', 21))
        h1_ema_l = ema(df_h1['close'], s5.get('H1_EMA_LONG', 55))
        h1_st, h1_st_dir = supertrend(df_h1, period=s5.get('H1_ST_PERIOD', 10), multiplier=s5.get('H1_ST_MULT', 3.0))

        if len(df_h1) < 5 or len(df_m15) < 50:
            _record_rejection(symbol, "S5-Insufficient bars", {"h1": len(df_h1), "m15": len(df_m15)})
            return

        h1_sig = pd.Series(index=df_h1.index, dtype=float)
        # Use the last closed H1 candle for trend filter
        h1_last = df_h1.iloc[-2]
        h1_ema_s_v = float(h1_ema_s.iloc[-2]) if not pd.isna(h1_ema_s.iloc[-2]) else None
        h1_ema_l_v = float(h1_ema_l.iloc[-2]) if not pd.isna(h1_ema_l.iloc[-2]) else None
        h1_dir_v = int(h1_st_dir.iloc[-2]) if not pd.isna(h1_st_dir.iloc[-2]) else 0

        if h1_ema_s_v is None or h1_ema_l_v is None or h1_dir_v == 0:
            _record_rejection(symbol, "S5-H1 filter not ready", {"ema_s": h1_ema_s_v, "ema_l": h1_ema_l_v, "st_dir": h1_dir_v})
            return

        h1_bull = h1_ema_s_v > h1_ema_l_v and h1_dir_v == 1
        h1_bear = h1_ema_s_v < h1_ema_l_v and h1_dir_v == -1
        if not (h1_bull or h1_bear):
            _record_rejection(symbol, "S5-No H1 trend", {"ema_s": h1_ema_s_v, "ema_l": h1_ema_l_v, "st_dir": h1_dir_v})
            return

        # M15 indicators using the last closed candle
        m15 = df_m15.copy()
        m15_ema_s = ema(m15['close'], s5.get('M15_EMA_SHORT', 21))
        m15_ema_l = ema(m15['close'], s5.get('M15_EMA_LONG', 55))
        m15['atr_s5'] = atr(m15, s5.get('ATR_PERIOD', 14))
        m15['rsi_s5'] = rsi(m15['close'], s5.get('RSI_PERIOD', 14))
        vol_lookback = s5.get('VOL_LOOKBACK', 10)
        m15['vol_avg'] = m15['volume'].rolling(vol_lookback).mean()

        sig = m15.iloc[-2]
        prev = m15.iloc[-3]
        curr = m15.iloc[-1]

        # Volatility guard based on ATR%
        atr_now = float(sig['atr_s5']) if not pd.isna(sig['atr_s5']) else 0.0
        close_now = float(sig['close'])
        atr_pct = (atr_now / close_now) if close_now > 0 else 0.0
        if not (s5.get('MIN_ATR_PCT', 0.003) <= atr_pct <= s5.get('MAX_ATR_PCT', 0.02)):
            _record_rejection(symbol, "S5-ATR% out of band", {"atr_pct": atr_pct})
            return

        # Volume spike check
        vol_avg = float(sig['vol_avg']) if not pd.isna(sig['vol_avg']) else 0.0
        vol_spike_mult = s5.get('VOL_SPIKE_MULT', 1.2)
        vol_ok = float(sig['volume']) >= (vol_spike_mult * vol_avg) if vol_avg > 0 else True

        m15_ema_s_v = float(m15_ema_s.iloc[-2]) if not pd.isna(m15_ema_s.iloc[-2]) else None
        m15_ema_l_v = float(m15_ema_l.iloc[-2]) if not pd.isna(m15_ema_l.iloc[-2]) else None
        rsi_now = float(sig['rsi_s5']) if not pd.isna(sig['rsi_s5']) else None

        if m15_ema_s_v is None or m15_ema_l_v is None or rsi_now is None:
            _record_rejection(symbol, "S5-M15 indicators not ready", {})
            return

        side = None
        # Execution confluence
        if h1_bull and sig['close'] > m15_ema_s_v > m15_ema_l_v and rsi_now < 70 and vol_ok:
            side = 'BUY'
        elif h1_bear and sig['close'] < m15_ema_s_v < m15_ema_l_v and rsi_now > 30 and vol_ok:
            side = 'SELL'
        else:
            _record_rejection(symbol, "S5-No M15 confluence", {
                "h1_bull": h1_bull, "h1_bear": h1_bear,
                "close": float(sig['close']), "ema_s": m15_ema_s_v, "ema_l": m15_ema_l_v,
                "rsi": rsi_now, "vol_ok": vol_ok
            }, signal_candle=sig)
            return

        # Entry at EMA21 retest (limit)
        entry_price = m15_ema_s_v
        # SL: structure/ATR hybrid
        stop_atr_mult = s5.get('STOP_ATR_MULT', 1.0)
        stop_min_atr_mult = s5.get('STOP_MIN_ATR_MULT', 1.0)
        stop_buffer_pct = s5.get('STOP_BUFFER_PCT', 0.0025)
        # Structure reference (signal candle wick)
        struct_low = float(sig['low'])
        struct_high = float(sig['high'])

        if side == 'BUY':
            sl_candidate = min(struct_low, entry_price) - max(stop_atr_mult * atr_now, stop_min_atr_mult * atr_now)
            sl_price = sl_candidate * (1 - 0)  # already buffered by ATR; apply price buffer below
            sl_price = min(sl_price, entry_price * (1 - stop_buffer_pct))
        else:
            sl_candidate = max(struct_high, entry_price) + max(stop_atr_mult * atr_now, stop_min_atr_mult * atr_now)
            sl_price = sl_candidate * (1 + 0)
            sl_price = max(sl_price, entry_price * (1 + stop_buffer_pct))

        distance = abs(entry_price - sl_price)
        if distance <= 0:
            _record_rejection(symbol, "S5-Invalid SL distance", {"entry": entry_price, "sl": sl_price}, signal_candle=sig)
            return

        # Sizing with fixed risk
        risk_usd = float(s5.get('RISK_USD', 0.5))
        ...
        # Place limit order at EMA21
        limit_order_resp = await asyncio.to_thread(place_limit_order_sync, symbol, side, final_qty, entry_price)
        order_id = str(limit_order_resp.get('orderId'))
        pending_order_id = f"{symbol}_{order_id}"

        # Expiry after N candles on 15m
        candle_duration = timeframe_to_timedelta("15m")
        expiry_candles = CONFIG.get("ORDER_EXPIRY_CANDLES", 2)
        expiry_time = m15.index[-1] + (candle_duration * (expiry_candles - 1))
        ...
        pending_meta = {
            ...
            "stop_price": sl_price,
            "take_price": 0.0,  # managed via trailing/partials (not on-exchange)
            "leverage": leverage,
            "risk_usdt": risk_usd,
        ...
        log.info(f"Placed pending limit order (S5): {pending_meta}")
        msg = (
            f"⏳ *New Pending Order: S5-Advanced*\\n\\n"
            f"**Symbol:** `{symbol}`\\n"
            f"**Side:** `{side}`\\n"
            f"**Entry (EMA21):** `{entry_price:.4f}`\\n"
            f"**SL:** `{sl_price:.4f}`\\n"
            f"**Qty:** `{final_qty}`\\n"
        ...
        await asyncio.to_thread(log_and_send_error, f"S5 evaluation failed for {symbol}", e)
...
async def run_test_order(strategy_id: int, symbol: str, full_test: bool):
    ...
    try:
        ...
        if df is None or df.empty:
            await asyncio.to_thread(send_telegram, f"❌ Cannot run test: no kline data for {symbol}.")
            return
        ...
        if strategy_id == 1:
            df_std = await asyncio.to_thread(calculate_all_indicators, df.copy())
            await evaluate_strategy_bb(symbol, df_std, **test_params)
        elif strategy_id == 2:
            df_std = await asyncio.to_thread(calculate_all_indicators, df.copy())
            await evaluate_strategy_supertrend(symbol, df_std, **test_params)
        elif strategy_id == 3:
            df_std = await asyncio.to_thread(calculate_all_indicators, df.copy())
            await evaluate_strategy_3(symbol, df_std, **test_params)
        elif strategy_id == 4:
            # Build Renko properly for S4 testing
            renko_df = await asyncio.to_thread(get_renko_data, df.copy(), symbol)
            if renko_df is None or renko_df.empty:
                await asyncio.to_thread(send_telegram, f"❌ S4 test failed: could not build Renko for {symbol}.")
                return
            df_s4 = await asyncio.to_thread(calculate_all_indicators, renko_df)
            await evaluate_strategy_4(symbol, df_s4, **test_params)
        elif strategy_id == 5:
            await evaluate_strategy_5(symbol)
        else:
